...
function clear_couchdb(task,cb){
    ...
    db = options.couchdb.db || ['carb','grid','state4k'].join('%2f')
    cdb += '/'+db
    if(options.couchdb.auth){
        user = options.couchdb.auth.username
        pass = options.couchdb.auth.password
    }
    console.log('clearing ',cdb,'from',first_doc_id,'to',last_doc_id)
    ...
    viewer(Object.assign({}
           ...
           ,function(e,docs){
               ...
               if(docs.rows.length>0){
                   ...
                   // nothing to delete, but make sure to sign off properly
                   cb(Error('no data'))
               }
