...
function file_worker(task,done){
    async.waterfall([check_file(task)
                    ...
                    ,couch_file
                    ,function(cb){
                         console.log('done saving '+task.file)
                         cb(null)
                     }
                    ]
                   ,function(e){
...
var file_q = async.queue(file_worker, 1) // []

var root = argv.root;
var subdir = argv.directory;

process.nextTick(function(){
    return file_q = populate_files(file_q,root,subdir)
})
...
var once = true
function save_header_once(task,cb){
    if(once){
        once = false
        console.log('saving header')
        var options = task.options
        //couchdb
        var chost = options.chost ? options.chost : chost
        var cport = options.cport ? options.cport : cport
