...
function couch_file(task,callback){
                    ...
                    ,function(v,i){
                         ...
                         if(geoms[geom_id]===undefined){
                             var geo = {type: "Feature"
                                       ,id: geom_id
                                       ,properties: _.clone(task.grid)
                                       ,geometry: task.grid.geom4326}
                             geoms[geom_id]=geo
                         }
                         doc.data=v
                         return doc
                     })
    // now to save  docs, the header stuff, and topology
    // first, get all existing topology, and merge
    ...
    request(topo_opts
           ,function(e,r,b){
                ...
                if(b !== undefined){
                    ...
                    var topology = JSON.parse(b)
                    _rev = topology._rev
                    delete topology._rev
                    delete topology._id
                ...
                if(_rev !== undefined) topo._rev=_rev
                topo_opts.body = JSON.stringify(topo)

                topo_opts.method='PUT'
                request(topo_opts
                       ,function(e,r,b){
                            if(e) return callback(e)
                            console.log(b)
    ...
    // now make sure that the header file exists
