...
function couch_file(task,callback){
    ...
    var put_opts = { 'headers': {}
               };
    put_opts.headers.authorization = 'Basic ' + new Buffer(cuser + ':' + cpass).toString('base64')
    put_opts.headers['Content-Type'] =  'application/json'

    // first make sure that the header file exists
    put_opts.body=JSON.stringify({'_id':'header'
                             ,'header':header
                             ,'unmapper':unmapper})
    put_opts.uri =  couch +'/header'
    ...
    put_opts.uri= couch +'/_bulk_docs'
    put_opts.method='POST'
    put_opts.body=[]
    var get_opts = _.clone(put_opts)
    get_opts.uri = couch+'/_all_docs'
    async.whilst(
        function(){
      ...
      ,function(whilst_callback){
           ...
           // slot in the ids to the get function
           var keys = _.map(chunk
                           ,function(doc){
                                hash[doc._id]=doc
                                return doc._id
                            });
           get_opts.body = JSON.stringify({'keys':keys})
           request(get_opts
                  ,function(e,r,b){
                       if(e) return whilst_callback(e)
                       var c = JSON.parse(b)
                       // put the revisions in the hash of docs
                       _.map(c.rows
                            ,function(row){
                                 hash[row.id]._rev=row.value.rev
                             });
                       put_opts.body = JSON.stringify({'docs':_.values(hash)})
                       request(put_opts
                              ,function(e,r,b){
                                   if(e){
